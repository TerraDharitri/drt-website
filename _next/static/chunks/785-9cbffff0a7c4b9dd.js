"use strict";
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [785],
  {
    8805: function (e, t, l) {
      var r = l(7437);
      t.Z = (e) => {
        let { className: t, classInner: l, hero: a, light: i, children: s } = e;
        return (0, r.jsx)("div", {
          className: "relative "
            .concat(a ? "pt-22 md:pt-18" : "", " ")
            .concat(t || ""),
          children: (0, r.jsx)("div", {
            className: "relative z-2 ".concat(l || ""),
            children: s,
          }),
        });
      };
    },
    9602: function (e, t, l) {
      var r = l(7437),
        a = l(4769);
      let i = {
        "arrow-down":
          "M7.044 9.204a.9.9 0 0 1 1.273 0l3.682 3.683 3.685-3.683a.9.9 0 0 1 1.172-.087l.101.087a.9.9 0 0 1 0 1.273l-4.32 4.32a.9.9 0 0 1-1.273 0l-4.32-4.32a.9.9 0 0 1 0-1.273z",
        "arrow-right":
          "M12.795 4.905l6.3 6.3.017.017.052.058-.069-.075c.036.036.069.074.1.114l.04.055.03.047.032.057.026.052.023.053a1.13 1.13 0 0 1 0 .835l-.023.052-.026.052-.033.058-.029.046-.04.056-.03.038-.052.058-.017.017-6.3 6.3c-.439.439-1.152.439-1.591 0s-.439-1.152 0-1.591l4.377-4.38H5.7c-.621 0-1.125-.504-1.125-1.125s.504-1.125 1.125-1.125h9.884l-4.379-4.38c-.399-.399-.436-1.024-.109-1.465l.109-.126c.439-.439 1.152-.439 1.591 0z",
        check:
          "M18.564 6.408a.9.9 0 0 1 1.273 1.273l-9.9 9.9a.9.9 0 0 1-1.273 0l-4.5-4.5a.9.9 0 0 1 1.273-1.273L9.3 15.671l9.264-9.263z",
        copyright:
          "M11.992 1.992c5.52 0 10 4.48 10 10s-4.48 10-10 10-10-4.48-10-10 4.48-10 10-10zm0 2c-4.41 0-8 3.59-8 8s3.59 8 8 8 8-3.59 8-8-3.59-8-8-8zm-.15 3.61c1.96 0 3.37 1.21 3.45 3.19h-1.79c-.02-.51-.35-1.6-1.63-1.66-.6.02-1.88.11-1.88 3 0 1.67.55 2.69 1.86 2.73 1.18-.06 1.6-.85 1.63-1.44h1.79c-.03 1.3-1.22 2.95-3.44 2.95-2.91 0-3.84-2.3-3.84-4.24 0-3.14 1.64-4.53 3.85-4.53z",
        eye: "M12 3.9c3.042 0 5.705 1.469 7.969 3.884.778.83 1.453 1.716 2.026 2.602l.71 1.211a.9.9 0 0 1 0 .805c-.127.253-.364.675-.71 1.211-.573.887-1.248 1.773-2.026 2.602C17.705 18.631 15.042 20.1 12 20.1s-5.705-1.469-7.969-3.884c-.778-.83-1.453-1.716-2.026-2.602l-.71-1.211a.9.9 0 0 1 0-.805c.127-.253.364-.675.71-1.211.573-.887 1.248-1.773 2.026-2.602C6.295 5.369 8.958 3.9 12 3.9zm0 1.8c-2.471 0-4.701 1.231-6.656 3.316a16.66 16.66 0 0 0-1.827 2.348l-.385.625-.008.011.008.013.274.451.111.173a16.66 16.66 0 0 0 1.827 2.348C7.299 17.069 9.529 18.3 12 18.3s4.701-1.231 6.656-3.316a16.66 16.66 0 0 0 1.827-2.348l.385-.625.006-.013-.006-.011-.274-.451-.111-.173a16.66 16.66 0 0 0-1.827-2.348C16.701 6.931 14.471 5.7 12 5.7zm0 2.7a3.6 3.6 0 1 1 0 7.2 3.6 3.6 0 0 1 0-7.2zm0 1.8a1.8 1.8 0 1 0 0 3.6 1.8 1.8 0 1 0 0-3.6z",
        facebook:
          "M2 12.056c0 4.972 3.611 9.106 8.333 9.944v-7.222h-2.5V12h2.5V9.778c0-2.5 1.611-3.888 3.889-3.888.722 0 1.5.111 2.222.222v2.556h-1.277c-1.223 0-1.5.611-1.5 1.389V12h2.667l-.444 2.778h-2.222V22C18.389 21.162 22 17.028 22 12.056 22 6.525 17.5 2 12 2S2 6.525 2 12.056z",
        linkedin:
          "M9.429 8.969h3.714v1.85c.535-1.064 1.907-2.02 3.968-2.02 3.951 0 4.889 2.118 4.889 6.004V22h-4v-6.312c0-2.213-.535-3.461-1.897-3.461-1.889 0-2.674 1.345-2.674 3.46V22h-4V8.969zM2.57 21.83h4V8.799h-4V21.83zM7.143 4.55a2.53 2.53 0 0 1-.753 1.802c-.484.481-1.138.75-1.82.748a2.59 2.59 0 0 1-1.818-.747A2.55 2.55 0 0 1 2 4.55c0-.677.27-1.325.753-1.803S3.89 2 4.571 2s1.336.269 1.819.747a2.54 2.54 0 0 1 .753 1.803z",
        twitter:
          "M23.356 4.96a9.33 9.33 0 0 1-2.675.733 4.67 4.67 0 0 0 2.048-2.578 9.3 9.3 0 0 1-2.958 1.13 4.66 4.66 0 0 0-5.601-.923 4.66 4.66 0 0 0-2.337 5.173 13.23 13.23 0 0 1-9.602-4.868c-.4.69-.63 1.49-.63 2.342a4.66 4.66 0 0 0 2.072 3.878c-.74-.024-1.463-.223-2.11-.583v.06A4.66 4.66 0 0 0 5.3 13.892c-.686.186-1.406.213-2.104.08a4.66 4.66 0 0 0 4.352 3.234 9.35 9.35 0 0 1-5.786 1.995 9.5 9.5 0 0 1-1.112-.065c2.129 1.369 4.608 2.096 7.14 2.093 8.57 0 13.255-7.098 13.255-13.254 0-.2-.005-.402-.014-.602a9.47 9.47 0 0 0 2.323-2.41l.002-.003z",
      };
      t.Z = (e) => {
        let { className: t, name: l, fill: s } = e;
        return (0, r.jsx)("svg", {
          className: (0, a.m6)("inline-flex w-6 h-6 ".concat(t || "")),
          width: 24,
          height: 24,
          viewBox: "0 0 24 24",
          children: (0, r.jsx)("path", { fill: s, d: i[l] }),
        });
      };
    },
    5996: function (e, t, l) {
      var r = l(7437),
        a = l(2265),
        i = l(6691),
        s = l.n(i);
      t.Z = (e) => {
        let { className: t, ...l } = e,
          [i, c] = (0, a.useState)(!1);
        return (0, r.jsx)(s(), {
          className: "inline-block align-top opacity-0 transition-opacity "
            .concat(i && "opacity-100", " ")
            .concat(t || ""),
          onLoad: () => c(!0),
          ...l,
        });
      };
    },
    8785: function (e, t, l) {
      l.d(t, {
        Z: function () {
          return k;
        },
      });
      var r = l(7437),
        a = l(2265),
        i = l(4033),
        s = l(4346),
        c = l(8805),
        o = l(1396),
        n = l.n(o),
        Image = l(5996),
        d = l(8811),
        m = l.n(d);
      let h = m()(() => l.e(859).then(l.bind(l, 8859)), {
        loadableGenerated: { webpack: () => [8859] },
        ssr: !1,
        loading: () =>
          (0, r.jsx)("div", {
            className:
              "w-8 h-8 rounded-full border border-greyscale-200 bg-white animate-pulse",
          }),
      });
      var g = (e) => (0, r.jsx)(h, { ...e });
      let x = [
          { id: "0", title: "Problems", url: "/problemsAndVision" },
          { id: "1", title: "Solution", url: "/solutions" },
          { id: "2", title: "Ecosystem", url: "/ecosystem" },
          { id: "3", title: "pREWA", url: "/pREWA" },
          { id: "4", title: "About Us", url: "/about-us" },
          { id: "5", title: "Resources", url: "/resources" },
          { id: "6", title: "Join Us", url: "/get-involved" },
        ],
        u = [
          {
            id: "0",
            title: "Platform",
            url: "/Platform",
            links: [
              { id: "0", title: "Home", url: "/" },
              { id: "1", title: "Problem & Vision", url: "/problemsAndVision" },
              { id: "2", title: "Our Solution", url: "/solutions" },
              { id: "3", title: "Ecosystem", url: "/ecosystem" },
            ],
          },
          {
            id: "1",
            title: "Token & Community",
            url: "/pREWA",
            links: [
              { id: "0", title: "pREWA Token", url: "/pREWA" },
              { id: "1", title: "Get Involved", url: "/get-involved" },
              { id: "2", title: "About Us", url: "/about-us" },
            ],
          },
          {
            id: "2",
            title: "Resources",
            url: "/resources",
            links: [
              { id: "0", title: "Resources", url: "/ResourcesPage" },
              { id: "1", title: "White Paper", url: "" },
              { id: "2", title: "Contact Us", url: "/get-involved" },
            ],
          },
          {
            id: "3",
            title: "Legal & Compliance",
            url: "",
            links: [
              { id: "0", title: "Privacy Policy", url: "" },
              { id: "1", title: "Terms of Service", url: "/" },
              { id: "2", title: "Disclaimers", url: "" },
              { id: "3", title: "Cookie Policy", url: "" },
              { id: "4", title: "pREWA Staking Terms", url: "" },
            ],
          },
        ];
      var f = (e) => {
          let { className: t, dark: l } = e,
            [i, c] = (0, a.useState)(!1);
          return (0, r.jsxs)("header", {
            className:
              "fixed top-0 left-0 right-0 z-20 border-b transition-colors duration-200 "
                .concat(t || "", " ")
                .concat(
                  l
                    ? "bg-secondary-300 border-white/[.09]"
                    : "bg-white dark:bg-dark-surface border-greyscale-200/60 dark:border-dark-border"
                ),
            children: [
              (0, r.jsxs)("div", {
                className: "container-md flex items-center h-22 md:h-18",
                children: [
                  (0, r.jsxs)(n(), {
                    className: "shrink-0 flex items-center mr-12 lg:mr-auto",
                    href: "/",
                    children: [
                      (0, r.jsx)(Image.Z, {
                        className: "w-7 opacity-100",
                        src: l
                          ? "images/graphics/Logo/Logo/Dharitri Logo Light.svg"
                          : "images/graphics/Logo/Logo/Dharitri Logo dark.svg",
                        width: 38,
                        height: 38,
                        alt: "Dharitri",
                      }),
                      (0, r.jsx)("span", {
                        className:
                          "ml-2 text-2xl font-bold transition-colors ".concat(
                            l
                              ? "text-greyscale-0"
                              : "text-greyscale-900 dark:text-dark-text-primary"
                          ),
                        children: "Dharitri",
                      }),
                    ],
                  }),
                  (0, r.jsxs)("div", {
                    className:
                      "flex items-center grow lg:fixed lg:top-0 lg:right-0 lg:bottom-0 lg:z-10 lg:w-80 lg:flex-col lg:items-stretch lg:py-20 lg:px-8 lg:pb-8 lg:transition-transform lg:translate-x-full "
                        .concat(i ? "!translate-x-0" : "", " ")
                        .concat(
                          l
                            ? "lg:bg-secondary-300"
                            : "lg:bg-greyscale-0 dark:lg:bg-dark-surface"
                        ),
                    children: [
                      (0, r.jsx)("nav", {
                        className:
                          "flex items-center space-x-4 lg:flex-col lg:items-stretch lg:space-x-0 lg:space-y-8",
                        children: x.map((e) =>
                          (0, r.jsx)(
                            n(),
                            {
                              className:
                                "px-2.5 py-1 text-base font-medium transition-colors lg:text-xl ".concat(
                                  l
                                    ? "text-greyscale-0 hover:text-primary-300"
                                    : "text-greyscale-900 dark:text-dark-text-primary hover:text-primary-100 dark:hover:text-primary-300"
                                ),
                              href: e.url,
                              children: e.title,
                            },
                            e.id
                          )
                        ),
                      }),
                      (0, r.jsxs)("div", {
                        className:
                          "flex items-center ml-auto space-x-3 lg:mt-auto lg:ml-0",
                        children: [
                          (0, r.jsx)(g, { size: "sm" }),
                          (0, r.jsx)(n(), {
                            className: "btn-primary btn-sm",
                            href: "/pREWA",
                            children: "Get pREWA Tokens",
                          }),
                        ],
                      }),
                    ],
                  }),
                  (0, r.jsx)("button", {
                    className:
                      "hidden relative z-20 flex-col justify-center items-start w-6 h-6 tap-highlight-color before:w-5 before:h-0.5 before:rounded-full before:transition-all after:w-5 after:h-0.5 after:rounded-full after:transition-all lg:flex "
                        .concat(
                          i
                            ? "before:rotate-45 before:translate-y-[0.37rem] after:-rotate-45 after:-translate-y-[0.37rem]"
                            : "",
                          " "
                        )
                        .concat(
                          l
                            ? "before:bg-greyscale-0 after:bg-greyscale-0"
                            : "before:bg-greyscale-900 after:bg-greyscale-900"
                        ),
                    onClick: () => {
                      c(!i),
                        i
                          ? (0, s.enablePageScroll)()
                          : (0, s.disablePageScroll)();
                    },
                    children: (0, r.jsx)("span", {
                      className: "w-5 h-0.5 my-1 rounded-full transition-all "
                        .concat(i ? "w-0 opacity-0" : "", " ")
                        .concat(l ? "bg-greyscale-0" : "bg-greyscale-900"),
                    }),
                  }),
                ],
              }),
              (0, r.jsx)("div", {
                className:
                  "hidden fixed inset-0 z-5 bg-greyscale-900/90 lg:block lg:invisible lg:opacity-0 lg:transition-all ".concat(
                    i ? "lg:!visible lg:!opacity-100" : ""
                  ),
                onClick: () => {
                  c(!1), (0, s.enablePageScroll)();
                },
              }),
            ],
          });
        },
        b = l(9602),
        p = l(9236),
        y = (e) => {
          let {} = e;
          return (0, r.jsx)("div", {
            className:
              "mt-auto py-16 md:pt-12 md:pb-6 bg-white dark:bg-dark-bg",
            children: (0, r.jsxs)("div", {
              className: "container",
              children: [
                (0, r.jsxs)("div", {
                  className: "flex mb-18 lg:block lg:mb-14 md:mb-8",
                  children: [
                    (0, r.jsxs)(n(), {
                      className:
                        "shrink-0 flex items-center mr-auto lg:mb-12 md:mb-8",
                      href: "/",
                      children: [
                        (0, r.jsx)(Image.Z, {
                          className: "w-7 opacity-100",
                          src: "images/graphics/Logo/Logo/Dharitri Logo dark.svg",
                          width: 38,
                          height: 38,
                          alt: "Dharitri",
                        }),
                        (0, r.jsx)("span", {
                          className:
                            "ml-2 text-2xl font-bold transition-colors text-greyscale-900 dark:text-dark-text-primary",
                          children: "Dharitri",
                        }),
                      ],
                    }),
                    (0, r.jsx)("div", {
                      className:
                        "flex w-full max-w-[50rem] 3xl:max-w-[45.625rem] lg:max-w-full md:flex-wrap md:-mt-5",
                      children: u.map((e) =>
                        (0, r.jsxs)(
                          "div",
                          {
                            className:
                              "grow pr-8 text-lg font-medium last:pr-0 md:w-1/2 md:mt-5 md:pr-5",
                            children: [
                              (0, r.jsx)(n(), {
                                className:
                                  "inline-block mb-4 text-[#666D80] dark:text-dark-text-secondary transition-colors hover:text-primary-100 dark:hover:text-primary-300 md:mb-2 md:text-base",
                                href: e.url,
                                children: e.title,
                              }),
                              (0, r.jsx)("div", {
                                className:
                                  "flex flex-col items-start space-y-3",
                                children: e.links.map((e) =>
                                  (0, r.jsx)(
                                    n(),
                                    {
                                      className:
                                        "transition-colors hover:text-primary-100 dark:hover:text-primary-300 dark:text-dark-text-primary md:text-base",
                                      href: e.url,
                                      children: e.title,
                                    },
                                    e.id
                                  )
                                ),
                              }),
                            ],
                          },
                          e.id
                        )
                      ),
                    }),
                  ],
                }),
                (0, r.jsxs)("div", {
                  className:
                    "flex justify-between items-center md:block md:pt-8 md:border-t md:border-greyscale-200 dark:md:border-dark-border",
                  children: [
                    (0, r.jsxs)("div", {
                      className:
                        "flex items-center text-lg text-[#666D80] dark:text-dark-text-secondary md:justify-center md:mb-4",
                      children: [
                        (0, r.jsx)(b.Z, {
                          className: "w-5 h-5 mr-1 fill-[#666D80]",
                          name: "copyright",
                        }),
                        "2025 Dharitri. All right reserved",
                      ],
                    }),
                    (0, r.jsx)("div", {
                      className: "flex space-x-4 md:justify-center",
                      children: p.U.map((e) =>
                        (0, r.jsx)(
                          "a",
                          {
                            className: "group text-0",
                            href: e.url,
                            target: "_blank",
                            rel: "noopener noreferrer",
                            children: (0, r.jsx)(b.Z, {
                              className:
                                "fill-[#666D80] transition-colors group-hover:fill-primary-100 md:w-5 md:h-5",
                              name: e.icon,
                            }),
                          },
                          e.id
                        )
                      ),
                    }),
                  ],
                }),
              ],
            }),
          });
        },
        k = (e) => {
          let { paddingTop: t, classHeader: l, darkHeader: o, children: n } = e,
            d = (0, i.usePathname)();
          return (
            (0, a.useEffect)(() => {
              (0, s.clearQueueScrollLocks)(), (0, s.enablePageScroll)();
            }, [d]),
            (0, r.jsxs)(r.Fragment, {
              children: [
                (0, r.jsx)(f, { className: l, dark: o }),
                (0, r.jsxs)(c.Z, {
                  className: "flex flex-col min-h-screen overflow-hidden",
                  classInner: "flex flex-col grow",
                  hero: t,
                  children: [n, (0, r.jsx)(y, {})],
                }),
              ],
            })
          );
        };
    },
    9236: function (e, t, l) {
      l.d(t, {
        U: function () {
          return r;
        },
      });
      let r = [
        {
          id: "0",
          icon: "linkedin",
          url: "https://www.linkedin.com/company/ui8",
        },
        { id: "1", icon: "twitter", url: "https://twitter.com/ui8" },
        { id: "2", icon: "facebook", url: "https://www.facebook.com/ui8.net/" },
      ];
    },
  },
]);
